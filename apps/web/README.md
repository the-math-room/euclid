...
- Multi-touch gesture interpretation in `GestureController` and `useWorkspaceGestures` (tap, pan, pinch-to-zoom, point drag).
...
- `src/GestureController.ts`: pure TypeScript state machine that manages multi-pointer interactions, coordinate panning, scaling, and tool selection. Completely decoupled from React.
- `src/useWorkspaceGestures.ts`: shallow event adapter hook. Binds DOM pointer events, calculates projection rects, and delegates events to the `GestureController`.
...
│   ├── useWorkspaceGestures (shallow event adapter hook)
│   │   └── GestureController (pure gesture state machine)
...
`App.tsx` should remain a thin composition root. If you need to add construction-level state or commands, add them to `useConstructionController`. If you need to add gesture handling, delegate to `GestureController` via `useWorkspaceGestures`. If you need new evaluation or editing logic, add it to `@euclid/geometry`.
...
- **Gesture Interpreter**: Keep all pointer/touch gesture state machine logic inside `GestureController.ts` and delegate to it via the shallow `useWorkspaceGestures.ts` hook. The gesture engine translates low-level events into high-level commands (`onAddPoint`, `onSelect`, `onMovePoint`, etc.) and calls the construction controller's callbacks.
