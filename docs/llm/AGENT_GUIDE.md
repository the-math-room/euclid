...
- `useWorkspaceGestures.ts` acts as a shallow React hook event adapter that delegates DOM pointer events to `GestureController.ts`. The `GestureController.ts` is a framework-agnostic pure state machine that manages pointer tracking, multi-touch calculations (pinch zoom, panning, dragging), and resolves gesture targeting priorities using declarative policies defined in `apps/web/src/construction/tools.ts`. All non-select tools must declare a policy.
